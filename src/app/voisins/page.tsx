import Link from 'next/link';
import { Shell } from '@/components/Shell';
import { NeighborsList } from '@/components/NeighborsList';
import { createClient } from '@/lib/supabase/server';
import { getServerPosition } from '@/lib/getServerPosition';

export const dynamic = 'force-dynamic';

export default async function VoisinsPage() {
  const supabase = createClient();
  const pos = getServerPosition();
  const {
    data: { user },
  } = await supabase.auth.getUser();

  if (!user) {
    return (
      <Shell>
        <div className="max-w-md mx-auto px-4 pt-24 lg:pt-40 text-center">
          <div className="text-5xl mb-3">👥</div>
          <h1 className="text-2xl font-black">Tes voisins.</h1>
          <p className="text-sm text-ink-700/60 mt-2">Connecte-toi pour découvrir et inviter tes voisins.</p>
          <Link
            href="/connexion"
            className="inline-block mt-6 px-8 py-3.5 bg-ink-900 text-cream-50 rounded-full font-extrabold text-sm shadow-lift"
          >
            Se connecter
          </Link>
        </div>
      </Shell>
    );
  }

  return (
    <Shell>
      <div className="max-w-2xl mx-auto">
        <header className="sticky top-0 z-30 bg-cream-50/95 backdrop-blur-xl border-b border-ink-900/5 px-4 lg:px-8 py-3 flex items-center gap-3">
          <Link href="/voisinage" className="text-sm font-bold text-ink-700/60 px-1">
            ←
          </Link>
          <div>
            <h1 className="text-xl lg:text-2xl font-black tracking-tight">Voisins</h1>
            <p className="text-[11px] text-ink-700/55">
              Invite-les pour devenir amis et pouvoir discuter
            </p>
          </div>
        </header>

        <div className="px-4 lg:px-8 py-4">
          <NeighborsList lat={pos.lat} lng={pos.lng} radius={2000} />
        </div>
      </div>
    </Shell>
  );
}
