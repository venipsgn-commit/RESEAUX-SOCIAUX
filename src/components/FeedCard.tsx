...
import { type NearbyPost, POST_TYPE_META, formatDistance, formatPrice, timeAgo } from '@/lib/types';

export function FeedCard({ post, likedByMe }: { post: NearbyPost; likedByMe: boolean }) {
    ...
    <article className="bg-surface rounded-2xl overflow-hidden border border-ink-900/5 shadow-soft">
      ...
      <div className="px-4 pt-3.5 pb-2.5 flex items-center gap-2.5">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-1.5 text-[12px] text-ink-700/55 mt-0.5">
            ...
            <span className="w-0.5 h-0.5 rounded-full bg-ink-700/40" />
            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" className="opacity-70" aria-hidden>
        ...
        <button aria-label="Options" className="w-9 h-9 -mr-1 rounded-full flex items-center justify-center text-ink-700/45 active:bg-ink-900/5 transition">
      ...
      >
        ...
        {post.event_at && (
          <div className="absolute bottom-3 right-3 bg-ink-900 text-white px-2.5 py-1 rounded-full text-[10px] font-bold">
            🗓 {new Date(post.event_at).toLocaleString('fr-FR', { hour: '2-digit', minute: '2-digit' })}
          </div>
        )}
      </Link>
      ...
      {hasCta && (
        <div className="px-4 pt-3">
          {post.type === 'sell' && (
            <Link href={`/post/${post.id}`} className="block text-center w-full py-2.5 bg-gradient-to-br from-forest-400 to-forest-600 text-white rounded-full font-extrabold text-sm shadow-soft">
              Acheter en escrow
            </Link>
          )}
          {post.type === 'service' && (
            <Link href={`/post/${post.id}`} className="inline-block px-3 py-1.5 bg-coral-500 text-white rounded-full text-xs font-bold shadow-soft">
              📅 Réserver
            </Link>
          )}
          {post.type === 'event' && (
            <Link href={`/post/${post.id}`} className="block text-center w-full py-2.5 bg-sunset-500 text-white rounded-full font-extrabold text-sm shadow-soft">
              Je viens 🙋
            </Link>
          )}
          {post.type === 'geolock' && (
            <Link href={`/post/${post.id}`} className="block text-center w-full py-2.5 bg-gradient-to-br from-lilac-300 to-lilac-500 text-white rounded-full font-extrabold text-sm shadow-soft">
      ...
      {/* PIED — J'aime / Commenter / Partager */}
      <ReactionBar
        variant="facebook"
        postId={post.id}
